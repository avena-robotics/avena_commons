...
class PepperDetector:
    ...
    def __init__(
        ...
        debug(f"PepperDetector: Utworzono detektor głównego systemu",
              message_logger=self._message_logger)

    def _get_pepper_vision_params(self, camera_number: int, mask_fragment: np.ndarray) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            section = self.pepper_config["camera_to_section"].get(camera_number, "top_left")
            pepper_type = self.pepper_config["pepper_detection"]["pepper_type"]
            reflective_nozzle = self.pepper_config["pepper_detection"]["reflective_nozzle"]
            ...
            debug(f"PepperDetector: Utworzono parametry pepper_vision dla kamery {camera_number}, sekcja: {section}",
                  message_logger=self._message_logger)

            return params

        except Exception as e:
            error(f"PepperDetector: Błąd tworzenia parametrów pepper_vision dla kamery {camera_number}: {e}",
                  message_logger=self._message_logger)
            return None
    ...
    def deserialize_fragment(self, fragment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if fragment_data.get('depth_fragment'):
                ...
                fragment['depth_fragment'] = cv2.imdecode(depth_array, cv2.IMREAD_UNCHANGED)

            # Dekoduj mask fragment
            if fragment_data.get('mask_fragment'):
                mask_bytes = base64.b64decode(fragment_data['mask_fragment'])
                mask_array = np.frombuffer(mask_bytes, dtype=np.uint8)
                fragment['mask_fragment'] = cv2.imdecode(mask_array, cv2.IMREAD_GRAYSCALE)

            return fragment

        except Exception as e:
            error(f"PepperDetector: Błąd deserializacji fragmentu: {e}",
                  message_logger=self._message_logger)
            return {}
    ...
    def detect_pepper_in_fragment(self, fragment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if camera_number is None or color_fragment is None or depth_fragment is None or mask_fragment is None:
                return {
                    'success': False,
            ...
            debug(f"PepperDetector: Fragment {fragment_id} z kamery {camera_number} - "
            ...
            error(f"PepperDetector: Błąd detekcji papryczki w fragmencie: {e}",
                  message_logger=self._message_logger)
            return {
                'success': False,
                'error': str(e),
    ...
    def process_fragments_from_cameras(self, fragments_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            debug(f"PepperDetector: Przetwarzanie {len(fragments_list)} fragmentów z różnych kamer",
                  message_logger=self._message_logger)
            ...
            for fragment in fragments_list:
                try:
                    ...
                    aggregated_results['cameras_processed'].add(fragment.get('camera_number'))
                    ...
                    error(f"PepperDetector: Błąd przetwarzania fragmentu: {fragment_error}",
                          message_logger=self._message_logger)
                    continue

            # Konwertuj set na listę dla JSON serialization
            aggregated_results['cameras_processed'] = list(aggregated_results['cameras_processed'])

            info(f"PepperDetector: Przetworzono fragmenty z {len(aggregated_results['cameras_processed'])} kamer, "
            ...
            error(f"PepperDetector: Błąd przetwarzania fragmentów z kamer: {e}",
                  message_logger=self._message_logger)
            return {
                'total_fragments': 0,
                'cameras_processed': [],
    ...
    def get_system_state(self, aggregated_results: Dict[str, Any]) -> CameraDetectionState:
        """
        ...
        try:
            ...
            if any(not result.get('detection_success', False) for result in aggregated_results.get('fragment_results', [])):
            ...
            error(f"PepperDetector: Błąd określania stanu systemu: {e}",
                  message_logger=self._message_logger)
            return CameraDetectionState.ERROR
    ...
    def reset_stats(self):
        ...
        debug("PepperDetector: Zresetowano statystyki detektora głównego systemu",
              message_logger=self._message_logger)