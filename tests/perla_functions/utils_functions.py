...
def pepper_mask_refinement(mask, params):
    ...
    mask_without_noise = cv2.morphologyEx(
        mask, cv2.MORPH_OPEN, np.ones(kernel_open, np.uint8), iterations=iterations_open
    )
    ...
    mask_without_noise = cv2.morphologyEx(
        mask_without_noise,
        cv2.MORPH_CLOSE,
        np.ones(kernel_close, np.uint8),
        iterations=iterations_close,
    )
    ...
    contours, _ = cv2.findContours(
        (mask_without_noise), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    for idx, contour in enumerate(contours):
        ...
        result = cv2.pointPolygonTest(
            contour, center_point, True
        )  # POPRAWIONE: używa dynamicznego środka
        ...
        if area > min_area:  # TODO: param
            ...
            if result > distance_threshold:
                print(
                    f"       Distance OK ({result:.1f} > {distance_threshold}) - CONTOUR ACCEPTED"
                )
                ...
                print(
                    f"       Distance NOT OK ({result:.1f} <= {distance_threshold}) - CONTOUR REJECTED"
                )
        else:
            print(
                f"       Area TOO SMALL ({area:.1f} <= {min_area}) - CONTOUR REJECTED"
            )
...
def refine_hole_mask_with_depth(hole_mask, pepper_mask, depth):
    ...
    near_hole_mask = cv2.dilate(
        hole_mask, np.ones((5, 5), np.uint8), iterations=1
    )  # TODO: param kernel
    far_hole_mask = cv2.dilate(
        hole_mask, np.ones((5, 5), np.uint8), iterations=4
    )  # TODO: param kernel
...
def get_mask_edge_area(mask):
    edge_area = mask - cv2.erode(
        mask, np.ones((8, 8), np.uint8), iterations=3
    )  # *255 #TODO: param kernel, iterations
    return edge_area
...
def nozzle_vector(nozzle_mask):
    ...
    nozzle_mask[:, 140:] = 0
    contours, _ = cv2.findContours(
        nozzle_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    if len(contours) == 0:
        return None
...
def create_seed_masks(image_rgb, params):
    ...
    mask_hsv = cv2.morphologyEx(
        mask_hsv, cv2.MORPH_CLOSE, (np.ones(hsv_close_1_params["kernel"], np.uint8))
    )
    mask_hsv = cv2.morphologyEx(
        mask_hsv, cv2.MORPH_DILATE, (np.ones(hsv_dilate_params["kernel"], np.uint8))
    )
    mask_hsv = cv2.morphologyEx(
        mask_hsv, cv2.MORPH_OPEN, (np.ones(hsv_open_params["kernel"], np.uint8))
    )
    mask_hsv = cv2.morphologyEx(
        mask_hsv, cv2.MORPH_CLOSE, (np.ones(hsv_close_2_params["kernel"], np.uint8))
    )
    ...
    mask_rgb = cv2.morphologyEx(
        mask_rgb, cv2.MORPH_DILATE, np.ones((rgb_dilate_params["kernel"]), np.uint8)
    )
    mask_rgb = cv2.morphologyEx(
        mask_rgb, cv2.MORPH_CLOSE, np.ones((rgb_close_1_params["kernel"]), np.uint8)
    )
    mask_rgb = cv2.morphologyEx(
        mask_rgb, cv2.MORPH_CLOSE, np.ones((rgb_close_2_params["kernel"]), np.uint8)
    )
    mask_rgb = cv2.morphologyEx(
        mask_rgb, cv2.MORPH_CLOSE, np.ones((rgb_close_3_params["kernel"]), np.uint8)
    )
...
def nozzle_in_pepper_mask(rgb, nozzle_mask, pepper_mask, section_select):
    ...
    canny_contour, _ = cv2.findContours(
    ...
    if len(internal_contours) > 0:
        ...
        for idx, contour in enumerate(internal_contours):
            ...
            print(
                f"     Contour {idx}: area={area:.1f}, distance_to_point={distance:.1f}"
            )
        ...
        if closest_contour is not None:
            ...
            print(
                f"   Selected closest contour: area={contour_area:.1f}, min_distance={min_distance:.1f}"
            )
            ...
            if contour_area < 0.4 * canny_area:
                print(
                    f"   ✅ CONTOUR ACCEPTED (area {contour_area:.1f} < {0.4 * canny_area:.1f})"
                )
                final_mask = np.zeros_like(nozzle_mask)
                ...
                print(
                    f"   ❌ CONTOUR REJECTED - TOO LARGE (area {contour_area:.1f} >= {0.4 * canny_area:.1f})"
                )
