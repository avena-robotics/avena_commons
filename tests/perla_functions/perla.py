...
class Perla(EventListener):
    ...
    def _init_pepper_detection(self):
        ...
        if self._auto_start_detection:
            info(
                "AUTO-START: Pepper detection będzie uruchomiony automatycznie po stabilizacji serwera",
                message_logger=self._message_logger,
            )
        else:
            debug(
                "AUTO-START: Pepper detection wyłączony - uruchom ręcznie przez HTTP API",
                message_logger=self._message_logger,
            )
    ...
    def _create_session_directory(self) -> str:
        ...
        try:
            ...
            session_dir = os.path.join(
                self._captures_config["base_directory"], f"session_{timestamp}"
            )
            ...
            info(
                f"CAPTURE: Utworzono katalog sesji: {session_dir}",
                message_logger=self._message_logger,
            )
            return session_dir
            ...
            error(
                f"CAPTURE: Błąd tworzenia katalogu sesji: {e}",
                message_logger=self._message_logger,
            )
            return None

    def _save_first_frame(
        self, rgb: np.ndarray, depth: np.ndarray, camera_number: int
    ) -> bool:
        """Zapisuje pierwszą ramkę z kamery (RGB + Depth)"""
        try:
            if (
                not self._captures_config["enable_capture"]
                or not self._captures_config["capture_first_frames"]
            ):
            ...
            rgb_filename = os.path.join(
                session_dir, f"camera{camera_number}_first_rgb.jpg"
            )
            cv2.imwrite(rgb_filename, rgb)

            # Zapisz Depth jako PNG (16-bit)
            depth_filename = os.path.join(
                session_dir, f"camera{camera_number}_first_depth.png"
            )
            ...
            info(
                f"CAPTURE: Zapisano pierwszą ramkę z kamery {camera_number}: {rgb_filename}, {depth_filename}",
                message_logger=self._message_logger,
            )
            return True
            ...
            error(
                f"CAPTURE: Błąd zapisywania pierwszej ramki z kamery {camera_number}: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    def _save_detection_results(self) -> bool:
        ...
        try:
            if (
                not self._captures_config["enable_capture"]
                or not self._captures_config["capture_results"]
            ):
                return True  # Capture wyłączony
            ...
            results_data = {
                ...
                "camera_states": {
                    str(k): v.name for k, v in self._state["camera_states"].items()
                },
                "camera_results": {},
            ...
            debug(
                f"CAPTURE: Zapisano wyniki detekcji: {results_filename}",
                message_logger=self._message_logger,
            )
            return True
            ...
            error(
                f"CAPTURE: Błąd zapisywania wyników detekcji: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    def _extract_camera_number(self, source: str) -> int:
        ...
        try:
            ...
            warning(
                f"Nie można wyekstraktować numeru kamery z: {source}",
                message_logger=self._message_logger,
            )
            return None
    ...
    def _deserialize_frame(self, frame_data: dict) -> tuple:
        ...
        try:
            ...
            error(
                f"Błąd deserializacji frame'a: {e}", message_logger=self._message_logger
            )
            return None, None
    ...
    def _load_nozzle_mask(self, camera_number: int) -> np.ndarray:
        ...
        try:
            ...
            if not mask_path or not os.path.exists(mask_path):
                warning(
                    f"Nie znaleziono maski nozzle dla kamery {camera_number}: {mask_path}",
                    message_logger=self._message_logger,
                )
                # Tworzę dummy maskę jako fallback
                dummy_mask = np.zeros((400, 640), dtype=np.uint8)
                cv2.circle(
                    dummy_mask, (320, 200), 50, 255, -1
                )  # Okrągła maska w centrum
            ...
            info(
                f"Załadowano maskę nozzle dla kamery {camera_number}: {mask.shape}",
                message_logger=self._message_logger,
            )
            return mask
            ...
            error(
                f"Błąd ładowania maski nozzle dla kamery {camera_number}: {e}",
                message_logger=self._message_logger,
            )
            return None
    ...
    def _get_pepper_params(self, camera_number: int) -> dict:
        ...
        try:
            ...
            section = self._configuration["camera_to_section"].get(
                camera_number, "top_left"
            )
            pepper_type = self._configuration["pepper_detection"]["pepper_type"]
            reflective_nozzle = self._configuration["pepper_detection"][
                "reflective_nozzle"
            ]
            ...
            debug(
                f"Utworzono parametry pepper_vision dla kamery {camera_number}, sekcja: {section}",
                message_logger=self._message_logger,
            )
            return params
            ...
            error(
                f"Błąd tworzenia parametrów pepper_vision dla kamery {camera_number}: {e}",
                message_logger=self._message_logger,
            )
            return None

    async def _process_pepper_detection(
        self, rgb: np.ndarray, depth: np.ndarray, camera_number: int
    ) -> dict:
        ...
        try:
            ...
            error(
                f"Błąd pepper detection dla kamery {camera_number}: {e}",
                message_logger=self._message_logger,
            )
            return {"success": False, "error": str(e), "camera_number": camera_number}
    ...
    def _log_state_change(self, camera_number=None, old_state=None, new_state=None):
        ...
        if camera_number:
            info(
                f"Camera {camera_number}: {old_state.name} → {new_state.name}",
                message_logger=self._message_logger,
            )
        else:
            info(
                f"System: {old_state.name} → {new_state.name}",
                message_logger=self._message_logger,
            )
    ...
    def _evaluate_system_state(self):
        ...
        found_count = sum(
            1 for s in camera_states if s == CameraDetectionState.PEPPER_FOUND
        )
        ...
        overflow_count = sum(
            1 for s in camera_states if s == CameraDetectionState.OVERFLOW_DETECTED
        )

        min_required = self._configuration["pepper_detection"][
            "min_cameras_for_success"
        ]
    ...
    async def _handle_camera_frame(self, event: Event) -> bool:
        ...
        try:
            ...
            if rgb is None or depth is None:
                error(
                    f"Nie udało się zdekodować frame'a z kamery {camera_number}",
                    message_logger=self._message_logger,
                )
            ...
            debug(
                f"Przetwarzanie frame'a z kamery {camera_number}: RGB {rgb.shape}, Depth {depth.shape}",
                message_logger=self._message_logger,
            )
            ...
            if result["success"]:
                ...
                if result["overflow_state"]:
                    self._update_camera_state(
                        camera_number, CameraDetectionState.OVERFLOW_DETECTED
                    )
                elif result["search_state"]:
                    self._update_camera_state(
                        camera_number, CameraDetectionState.PEPPER_FOUND
                    )
                    self._update_camera_state(camera_number, CameraDetectionState.READY)
                else:
                    self._update_camera_state(
                        camera_number, CameraDetectionState.NO_PEPPER
                    )
                ...
                info(
                    f"Pepper detection kamera {camera_number}: papryczka={result['search_state']}, overflow={result['overflow_state']}",
                    message_logger=self._message_logger,
                )
            else:
                error(
                    f"Pepper detection błąd kamera {camera_number}: {result.get('error', 'nieznany')}",
                    message_logger=self._message_logger,
                )
                self._update_camera_state(camera_number, CameraDetectionState.ERROR)
            ...
            error(
                f"Błąd obsługi camera_frame: {e}", message_logger=self._message_logger
            )
            return False
    ...
    async def _start_pepper_detection(self, event: Event) -> bool:
        ...
        try:
            if self._state["detection_enabled"]:
                warning(
                    "Pepper detection już włączony", message_logger=self._message_logger
                )
                return True

            info(
                "Uruchamianie pepper detection...", message_logger=self._message_logger
            )

            # NOWE: Rozpocznij inicjalizację kamer (FSM)
            if self._camera_init_fsm == CameraInitFSM.IDLE:
                info(
                    "Rozpoczynam inicjalizację kamer...",
                    message_logger=self._message_logger,
                )
                self._camera_init_fsm = (
                    CameraInitFSM.IDLE
                )  # Będzie obsłużone w _check_local_data
            ...
            self._first_frames_captured = {1: False, 2: False, 3: False, 4: False}
            self._captures_config["session_directory"] = (
                None  # Wymusi utworzenie nowej sesji
            )
            ...
            info(
                "Pepper detection uruchomiony pomyślnie",
                message_logger=self._message_logger,
            )
            return True
            ...
            error(
                f"Błąd uruchamiania pepper detection: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    async def _stop_pepper_detection(self, event: Event) -> bool:
        ...
        try:
            if not self._state["detection_enabled"]:
                warning(
                    "Pepper detection już wyłączony",
                    message_logger=self._message_logger,
                )
                return True

            info(
                "Zatrzymywanie pepper detection...", message_logger=self._message_logger
            )
            ...
            detection_duration = (
                time.time() - self._state["detection_start_time"]
                if self._state["detection_start_time"]
                else 0
            )
            stats = self._state["pepper_stats"]

            info(
                f"Podsumowanie pepper detection - czas: {detection_duration:.2f}s, "
            ...
            info(
                "Pepper detection zatrzymany pomyślnie",
                message_logger=self._message_logger,
            )
            return True
            ...
            error(
                f"Błąd zatrzymywania pepper detection: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    async def _get_pepper_status(self, event: Event) -> dict:
        ...
        try:
            return {
                ...
                "detection_enabled": self._state["detection_enabled"],
                "camera_states": {
                    k: v.name for k, v in self._state["camera_states"].items()
                },
                "camera_results": self._state["camera_results"],
                "pepper_stats": self._state["pepper_stats"],
                "detection_duration": time.time() - self._state["detection_start_time"]
                if self._state["detection_start_time"]
                else 0,
            }
        except Exception as e:
            error(
                f"Błąd pobierania statusu pepper detection: {e}",
                message_logger=self._message_logger,
            )
            return {"error": str(e)}
    ...
    async def _analyze_event(self, event: Event) -> bool:
        ...
        try:
            ...
            # Obsługa potwiedzeń inicjalizacji kamer (wzór z TestEventServer)
                ...
                event.source.startswith("camera") and event.event_type == "camera_init"
            ):
                return await self._handle_camera_init_response(event)
            elif (
                event.source.startswith("camera") and event.event_type == "camera_start"
            ):
    ...
    async def _check_local_data(self):
        ...
        try:
            ...
            if (
                self._state["detection_enabled"]
                and self._state["detection_start_time"]
                and time.time() - self._state["detection_start_time"]
                > self._configuration["pepper_detection"]["timeout_seconds"]
            ):
                warning(
                    "Timeout pepper detection - zatrzymywanie...",
                    message_logger=self._message_logger,
                )
            ...
            error(
                f"Błąd sprawdzania danych lokalnych: {e}",
                message_logger=self._message_logger,
            )
    ...
    async def _handle_camera_init_fsm(self):
        ...
        try:
            ...
            if (
                self._camera_init_fsm == CameraInitFSM.IDLE
                and not self._state.get("detection_enabled", False)
                and hasattr(self, "_auto_start_detection")
                and self._auto_start_detection
            ):
                info(
                    "AUTO-START: Uruchamianie pepper detection po stabilizacji serwera...",
                    message_logger=self._message_logger,
                )
                await self._start_pepper_detection(None)
                self._auto_start_detection = False  # Auto-start tylko raz

            elif self._camera_init_fsm == CameraInitFSM.IDLE and self._state.get(
                "detection_enabled", False
            ):
                # Rozpocznij inicjalizację - wyślij camera_init do wszystkich kamer
                info(
                    "FSM: Wysyłanie camera_init do wszystkich kamer...",
                    message_logger=self._message_logger,
                )
                ...
                for camera_num, config in self._cameras_config.items():
                    try:
                        ...
                        debug(
                            f"FSM: Wysłano camera_init do camera{camera_num} (port {config['port']})",
                            message_logger=self._message_logger,
                        )

                    except Exception as e:
                        error(
                            f"FSM: Błąd wysyłania camera_init do camera{camera_num}: {e}",
                            message_logger=self._message_logger,
                        )
                ...
                info(
                    "FSM: Wszyscy odpowiedzieli na camera_init, wysyłanie camera_start...",
                    message_logger=self._message_logger,
                )
                ...
                for camera_num, config in self._cameras_config.items():
                    try:
                        ...
                        debug(
                            f"FSM: Wysłano camera_start do camera{camera_num} (port {config['port']})",
                            message_logger=self._message_logger,
                        )

                    except Exception as e:
                        error(
                            f"FSM: Błąd wysyłania camera_start do camera{camera_num}: {e}",
                            message_logger=self._message_logger,
                        )
            ...
            error(
                f"FSM: Błąd obsługi camera init FSM: {e}",
                message_logger=self._message_logger,
            )
    ...
    async def _handle_camera_init_response(self, event: Event) -> bool:
        ...
        try:
            ...
            info(
                f"FSM: Otrzymano camera_init od camera{camera_num}",
                message_logger=self._message_logger,
            )
            ...
            if self._camera_init_fsm == CameraInitFSM.INIT_SENT:
                ...
                if len(self._camera_init_responses) >= len(self._cameras_config):
                    info(
                        "FSM: Wszystkie kamery odpowiedziały na camera_init",
                        message_logger=self._message_logger,
                    )
            ...
            error(
                f"FSM: Błąd obsługi camera_init response: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    async def _handle_camera_start_response(self, event: Event) -> bool:
        ...
        try:
            camera_num = self._extract_camera_number(event.source)
            info(
                f"FSM: Otrzymano camera_start od camera{camera_num}",
                message_logger=self._message_logger,
            )
            ...
            if self._camera_init_fsm == CameraInitFSM.START_SENT:
                ...
                if len(self._camera_init_responses) >= len(self._cameras_config):
                    info(
                        "FSM: Wszystkie kamery gotowe - pepper detection może rozpocząć!",
                        message_logger=self._message_logger,
                    )
            ...
            error(
                f"FSM: Błąd obsługi camera_start response: {e}",
                message_logger=self._message_logger,
            )
            return False
    ...
    async def _stop_cameras(self):
        ...
        try:
            info(
                "FSM: Zatrzymywanie kamer - resetuję FSM inicjalizacji...",
                message_logger=self._message_logger,
            )
            ...
            info(
                "FSM: Kamery zostaną zatrzymane przez brak eventów pepper detection",
                message_logger=self._message_logger,
            )

        except Exception as e:
            error(
                f"FSM: Błąd zatrzymywania kamer: {e}",
                message_logger=self._message_logger,
            )
