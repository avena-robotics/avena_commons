...
def depth_measurement_in_mask(depth, mask):
    ...
    if non_zero_mask_depth_vals.size == 0: return 0, 0, debug
...
def if_pepper_is_filled(inner_zone_median, inner_zone_non_zero_perc, outer_zone_median, outer_zone_median_start, params):
    debug = {}
    
    func_params = params["if_pepper_is_filled_config"]
    
    debug["diff"] = inner_zone_median - (outer_zone_median_start+outer_zone_median)/2
    
    if abs(outer_zone_median - outer_zone_median_start) >= func_params["max_outer_diff"]: #TODO: param
        # print("becouse diff outer", outer_zone_median - outer_zone_median_start)
        return True, debug
    
    if inner_zone_non_zero_perc <= func_params["min_inner_zone_non_zero_perc"]: #TODO: param
        return False, debug
    
    if inner_zone_median - (outer_zone_median_start+outer_zone_median)/2 <= func_params["min_inner_to_outer_diff"]: # TODO: param 
...
def overflow_detection(rgb, overflow_mask, hsv_range, max_value, bias = 0):
    ...
    white_percentage, debug_mask = get_white_percentage_in_mask(rgb, overflow_mask, hsv_range)
    ...
    if white_percentage >= (0.05 + bias): return True, debug #TODO: param
    return False, debug