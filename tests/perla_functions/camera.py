...
class Camera(EventListener):
    ...
    def __init__(
        ...
        if self._configuration.get("camera_ip_per_number", None) is None or self._configuration["camera_ip_per_number"].get(self.camera_number, None) is None:
            error(f"EVENT_LISTENER_INIT: Brak konfiguracji CAMERA_IP_PER_NUMBER dla kamery {self.camera_number}", self._message_logger)
            raise ValueError(f"Brak konfiguracji CAMERA_IP_PER_NUMBER dla kamery {self.camera_number}")
        self.camera_address = self._configuration["camera_ip_per_number"][self.camera_number]
        ...
        debug(f"EVENT_LISTENER_INIT: Event listener kamery {self.camera_number} został zainicjalizowany dla ip {self.camera_address}", self._message_logger)
    ...
    async def _check_local_data(self):
        ...
        if self.camera_running:
            ...
            try:
                ...
                if frames is None:
                    error(f"Timeout przy pobieraniu ramek z kamery {self.camera_number} (IP: {self.camera_address})", self._message_logger)
                    return
                ...
                if len(rois) > 0:
                    info(f"Wytworzono {len(rois)} ROI, wysyłanie ramek", self._message_logger)
                    await self._send_frames(rois)
                else:
                    debug("Brak ROI - sprawdź poprzednie komunikaty diagnostyczne", self._message_logger)
                    
            except Exception as pipeline_error:
                error(f"Błąd pipeline kamery {self.camera_number}: {pipeline_error}", self._message_logger)
                error(f"Kamera IP: {self.camera_address}, Port pipeline: prawdopodobnie 8090", self._message_logger)
                # Możliwe przyczyny: problema sieciowe, kamera offline, konflikt IP
        else:
            debug("Kamera nie jest uruchomiona - pomijanie sprawdzania danych", self._message_logger)
    ...
    async def _init_camera(self, event: Event):
        ...
        try:
            ...
            debug(f"CAMERTA_INIT: Otwarta kamera {self.camera_number} na ip {self.camera_address}", self._message_logger)
            debug(f"CAMERTA_INIT: camera info: {dev.get_device_info()}", self._message_logger)
        except Exception as e:
            error(f"CAMERTA_INIT: Błąd podczas otwierania kamery {self.camera_number} na ip {self.camera_address}: {e}", self._message_logger)
            raise e
        ...
        try:
            ...
            if disparity_settings:
                ...
                if not dev.is_property_supported(disparity_pid, OBPermissionType.PERMISSION_READ_WRITE):
                    error(f"CAMERTA_INIT: Zmiana trybu zakresu dysparycji nie jest obsługiwana w kamerze {self.camera_number}. Sprawdź wersję firmware.", self._message_logger)
                else:
                    ...
                    if desired_mode_name:
                        ...
                        debug(f"CAMERTA_INIT: Aktualny tryb zakresu dysparycji: {current_mode.name}", self._message_logger)
                        ...
                        for i in range(available_modes.get_count()):
                            ...
                            if mode.name == desired_mode_name:
                                ...
                                debug(f"CAMERTA_INIT: Ustawiono tryb zakresu dysparycji na {mode.name}", self._message_logger)
                                is_mode_set = True
                                break
                        
                        if not is_mode_set:
                            error(f"CAMERTA_INIT: Żądany tryb zakresu dysparycji '{desired_mode_name}' nie został znaleziony. Dostępne tryby: {available_mode_names}. Używam bieżącego: {current_mode.name}", self._message_logger)

            # Set Disparity Search Offset
            offset_pid = OBPropertyID.OB_STRUCT_DISPARITY_SEARCH_OFFSET
            if not dev.is_property_supported(offset_pid, OBPermissionType.PERMISSION_READ_WRITE):
                 debug(f"CAMERTA_INIT: Przesunięcie wyszukiwania dysparycji nie jest obsługiwane w kamerze {self.camera_number}", self._message_logger)
            else:
                ...
                if disparity_offset and disparity_offset > 0:
                    ...
                    debug(f"CAMERTA_INIT: Ustawiono przesunięcie wyszukiwania dysparycji na {disparity_offset}", self._message_logger)
        except AttributeError:
            error("CAMERTA_INIT: Używana wersja pyorbbecsdk nie obsługuje zmiany trybu dysparycji. Wymagana jest wersja >= 2.2.x. Ta funkcja zostanie pominięta.", self._message_logger)
        ...
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_COLOR_EXPOSURE_INT na {color_exposure}", self._message_logger)

        color_gain = color_settings.get("gain", 10)
        dev.set_int_property(OBPropertyID.OB_PROP_COLOR_GAIN_INT, color_gain)
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_COLOR_GAIN_INT na {color_gain}", self._message_logger)

        color_white_balance = color_settings.get("white_balance", 4000)
        dev.set_int_property(OBPropertyID.OB_PROP_COLOR_WHITE_BALANCE_INT, color_white_balance)
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_COLOR_WHITE_BALANCE_INT na {color_white_balance}", self._message_logger)
        ...
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_DEPTH_EXPOSURE_INT na {depth_exposure}", self._message_logger)
        
        depth_gain = depth_settings.get("gain", 10)
        dev.set_int_property(OBPropertyID.OB_PROP_DEPTH_GAIN_INT, depth_gain)
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_DEPTH_GAIN_INT na {depth_gain}", self._message_logger)

        laser_power = depth_settings.get("laser_power", 5)
        dev.set_int_property(OBPropertyID.OB_PROP_LASER_POWER_LEVEL_CONTROL_INT, laser_power)
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_LASER_POWER_LEVEL_CONTROL_INT na {laser_power}", self._message_logger)
        
        # Disparity
        disparity_to_depth = camera_settings.get("disparity_to_depth", True)
        dev.set_bool_property(OBPropertyID.OB_PROP_DISPARITY_TO_DEPTH_BOOL, disparity_to_depth)
        debug(f"CAMERTA_INIT: Ustawiono OB_PROP_DISPARITY_TO_DEPTH_BOOL na {disparity_to_depth}", self._message_logger)
        ...
        if use_hardware_align:
            ...
            debug(f"CAMERTA_INIT: Ustawiono tryb wyrównania na HW_MODE", self._message_logger)
            ...
            match color_settings.get("format", "BGR"):
                ...
                case _:
                    error(f"CAMERTA_INIT: Nieznany format koloru: {color_settings.get('format', 'BGR')} - urzyty BGR", self._message_logger)
            ...
            debug(f"CAMERTA_INIT: Wyszukuję profil koloru dla {width}x{height}@{fps} {color_format}", self._message_logger)
            color_profile = color_profile_list.get_video_stream_profile(width, height, color_format, fps)
            
            if not color_profile:
                 error(f"CAMERTA_INIT: Nie znaleziono pasującego profilu koloru dla {width}x{height}@{fps} {color_format}", self._message_logger)
                ...
                if not hw_d2c_profile_list or len(hw_d2c_profile_list) == 0:
                    error(f"CAMERTA_INIT: Nie znaleziono zgodnego profilu głębi dla wyrównania sprzętowego.", self._message_logger)
                    ...
                    debug(f"CAMERTA_INIT: Włączono strumień głębi (HW Align): {depth_profile}", self._message_logger)
                    self.camera_config.enable_stream(color_profile)
                    debug(f"CAMERTA_INIT: Włączono strumień koloru (HW Align): {color_profile}", self._message_logger)
        ...
        if not use_hardware_align:
            ...
            if camera_settings.get("align", True):
                ...
                debug(f"CAMERTA_INIT: Utworzono programowy filtr wyrównujący do strumienia koloru", self._message_logger)
            ...
            if color_profile:
                self.camera_config.enable_stream(color_profile)
                debug(f"CAMERTA_INIT: Włączono strumień koloru: {color_profile}", self._message_logger)
            else:
                error(f"CAMERTA_INIT: Nie znaleziono pasującego profilu koloru.", self._message_logger)
            ...
            if depth_profile:
                self.camera_config.enable_stream(depth_profile)
                debug(f"CAMERTA_INIT: Włączono strumień głębi: {depth_profile}", self._message_logger)
            else:
                error(f"CAMERTA_INIT: Nie znaleziono pasującego profilu głębi.", self._message_logger)
        ...
        info(f"CAMERTA_INIT: Konfiguracja kamery {self.camera_number} została zakończona", self._message_logger)
    ...
    async def _start_camera(self, event: Event):
        ...
        if self.pipeline is None:
            error("CAMERTA_INIT: Pipeline nie został zainicjalizowany", self._message_logger)
            raise ValueError("Pipeline nie został zainicjalizowany")
        
        if self.camera_config is None:
            error("CAMERTA_INIT: Config nie został zainicjalizowany", self._message_logger)
    ...
    async def _send_frames(self, rois):
        ...
        for destination in self._configuration["destinations"]:
            print("send frame to: ", destination, self._configuration["destinations"][destination]["address"], self._configuration["destinations"][destination]["port"])
            roi = rois[self._configuration["destinations"][destination]["roi_number"]]
            
            await self._event(
                destination=f"{destination}",
                destination_address=self._configuration["destinations"][destination]["address"],
                destination_port=self._configuration["destinations"][destination]["port"],
    ...
    def _process_frames(self, frames):
        ...
        try:
            ...
            if frame_color is not None:
                ...
                debug(f"Nowa ramka kolorowa zapisana w buforze (timestamp: {current_time:.0f})", self._message_logger)
                
            if frame_depth is not None:
                self.latest_depth_frame = frame_depth
                self.last_depth_timestamp = current_time
                debug(f"Nowa ramka głębi zapisana w buforze (timestamp: {current_time:.0f})", self._message_logger)
            
            # Debug: sprawdź dostępność ramek
            debug(f"Frame check - color: {frame_color is not None}, depth: {frame_depth is not None}", self._message_logger)
            debug(f"Buffer status - color: {self.latest_color_frame is not None}, depth: {self.latest_depth_frame is not None}", self._message_logger)
            ...
            if use_color_frame is None or use_depth_frame is None:
                debug(f"Brak ramek w buforach - color: {use_color_frame is not None}, depth: {use_depth_frame is not None}", self._message_logger)
                return rois
            ...
            if time_diff > self.frame_sync_timeout:
                debug(f"Ramki zbyt desynchronizowane: różnica {time_diff:.0f}ms (limit: {self.frame_sync_timeout}ms)", self._message_logger)
                # Still try to process, but warn about sync
                info(f"Przetwarzanie desynchronizowanych ramek (różnica: {time_diff:.0f}ms)", self._message_logger)
            
            # Process color frame
            color_image = None
            if use_color_frame.get_format() == OBFormat.MJPG:
                debug(f"Dekodowanie MJPG ramki kolorowej {use_color_frame.get_width()}x{use_color_frame.get_height()}", self._message_logger)
                color_image = cv2.imdecode(use_color_frame.get_data(), cv2.IMREAD_COLOR)
                if color_image is None:
                    error(f"Błąd dekodowania MJPG ramki kolorowej", self._message_logger)
                    # Try to interpret as raw data
                    try:
                        color_image = use_color_frame.get_data().reshape((use_color_frame.get_height(), use_color_frame.get_width(), 3))
                        debug(f"Użyto fallback raw data dla ramki kolorowej", self._message_logger)
                    except Exception as fallback_error:
                        error(f"Fallback raw data też zawiódł: {fallback_error}", self._message_logger)
                        return rois
            else:
                color_image = cv2.cvtColor(use_color_frame.get_data().reshape((use_color_frame.get_height(), use_color_frame.get_width(), 3)), cv2.COLOR_RGB2BGR)
            
            # Process depth frame
            debug(f"Przetwarzanie ramki głębi z bufora {use_depth_frame.get_width()}x{use_depth_frame.get_height()}", self._message_logger)
            depth_data = np.frombuffer(bytes(use_depth_frame.get_data()), dtype=np.uint16)
            depth_image = depth_data.reshape((use_depth_frame.get_height(), use_depth_frame.get_width()))
                
            # Create ROIs from synchronized frames
            if color_image is not None and depth_image is not None:
                debug(f"Tworzenie ROI z zsynchronizowanych ramek - {len(self._configuration['rois'])} regionów", self._message_logger)
                ...
                info(f"Utworzono {len(rois)} ROI z zsynchronizowanych ramek (sync diff: {time_diff:.0f}ms)", self._message_logger)
            else:
                error(f"Błąd tworzenia obrazów z zsynchronizowanych ramek", self._message_logger)
