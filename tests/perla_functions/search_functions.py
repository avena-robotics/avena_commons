...
from .utils_functions import pepper_mask_refinement, convex_hull_mask, get_mask_edge_area, convex_hull_joined_masks, nozzle_in_pepper_mask
...
def pepper_presence(rgb, nozzle_mask, section= "top_left"):
    ...
    for point in messurment_points:
        ...
        if mask_sum < 10: error_count += 1 #TODO: param 
...
def pepper_mask(rgb, nozzle_mask, params):
    ...
    if params["reflective_nozzle"] == True:
        print(f"🔧 PEPPER_MASK: reflective_nozzle=True, wywołuję nozzle_in_pepper_mask dla sekcji '{params['section']}'")
        nozzle_in_pepper_mask_var = nozzle_in_pepper_mask(rgb, nozzle_mask, mask, params["section"])
        if nozzle_in_pepper_mask_var is not None and np.max(nozzle_in_pepper_mask_var) > 0:
            print(f"🔧 PEPPER_MASK: nozzle_in_pepper_mask zwrócił maskę z {np.count_nonzero(nozzle_in_pepper_mask_var)} pikselami")
            ...
            nozzle_outside_pepper = nozzle_mask - cv2.bitwise_and(nozzle_mask, nozzle_in_pepper_mask_var)
...
def hole_detection(rgb, pepper_mask, params):
    ...
    threshold_param_value = (((np.median(l_non_zero_normalized)*threshold_param) / 255) * np.max(l_non_zero) ) + np.min(l_non_zero)
    debug['threshold_param'] = threshold_param_value
    l_threshold = cv2.threshold(l_clahe, threshold_param_value, 255, cv2.THRESH_BINARY)[1]
    ...
    l_threshold_morph = cv2.morphologyEx(l_threshold, cv2.MORPH_OPEN, np.ones(open_on_l_params["kernel"], np.uint8), iterations=open_on_l_params["iterations"]) 
    ...
    center_area_morph = cv2.morphologyEx(center_area, cv2.MORPH_OPEN, np.ones(open_on_center_params["kernel"], np.uint8), iterations=open_on_center_params["iterations"])
    ...
    hole_mask_raw_center_morph = cv2.morphologyEx(hole_mask_raw_center, cv2.MORPH_OPEN, np.ones(open_on_center_raw_params["kernel"], np.uint8), iterations=open_on_center_raw_params["iterations"])
    debug["hole_mask_raw_center_morph"] = hole_mask_raw_center_morph
    
    #TODO: Filter to keep only close to center
    good_contours = []
    hole_contours, _ = cv2.findContours(hole_mask_raw_center_morph, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    hole_mask_after_morph = cv2.morphologyEx(hole_mask_raw_center_morph, cv2.MORPH_CLOSE, np.ones(close_params["kernel"], np.uint8), iterations=close_params["iterations"])
    debug['hole_mask_after_morph'] = hole_mask_after_morph
    
    #CONTOURS FINDING
    hole_contours, _ = cv2.findContours(hole_mask_after_morph, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if len(hole_contour) > 0: cv2.fillPoly(hole_mask, hole_contour, 255) 
...
def split_nozzle_mask(nozzle_mask, hole_mask, pepper_mask, vectors, section="top_right"):
    debug={}

    raw_nozzle_in_pepper_mask = cv2.bitwise_and(nozzle_mask, pepper_mask)
    nozzle_in_pepper_mask = cv2.morphologyEx(raw_nozzle_in_pepper_mask, cv2.MORPH_OPEN, np.ones((2, 2), np.uint8)) #TODO: param
    ...
    contours, _ = cv2.findContours(nozzle_in_hole_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if contours:
        ...
        rotated_mask = cv2.warpAffine(nozzle_in_hole_mask, M, (nozzle_in_hole_mask.shape[1], nozzle_in_hole_mask.shape[0]))
        ...
        left_split_original = cv2.warpAffine(left_split, M_inverse, (left_split.shape[1], left_split.shape[0]))
        right_split_original = cv2.warpAffine(right_split, M_inverse, (right_split.shape[1], right_split.shape[0]))
        ...
        #Nozzle outside mask
        nozzle_outside_mask = cv2.bitwise_and(nozzle_mask, pepper_mask_hull) - cv2.bitwise_and(nozzle_in_hole_mask, nozzle_mask)
        nozzle_outside_mask = nozzle_outside_mask - cv2.bitwise_and(nozzle_in_pepper_mask, nozzle_outside_mask)
        
        nozzle_outside_mask = cv2.morphologyEx(nozzle_outside_mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8)) #TODO: param
        
        match section:
            case "top_left" | "bottom_left":
                if left_split_original is not None:
                    nozzle_in_hole_mask = nozzle_in_hole_mask - cv2.bitwise_and(nozzle_in_hole_mask, left_split_original)
                    nozzle_outside_mask = nozzle_outside_mask + left_split_original
            case "top_right" | "bottom_right":
                if right_split_original is not None:
                    nozzle_in_hole_mask = nozzle_in_hole_mask - cv2.bitwise_and(nozzle_in_hole_mask, right_split_original)
                    nozzle_outside_mask = nozzle_outside_mask + right_split_original
            case _:
                pass
                
        nozzle_in_hole_mask = cv2.morphologyEx(nozzle_in_hole_mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8)) #TODO: param
    else:
        nozzle_in_hole_mask = np.zeros_like(nozzle_mask)
        nozzle_outside_mask = np.zeros_like(nozzle_mask)
...
def depth_measurement_zones(hole_mask, pepper_mask, nozzle_in_hole_mask, nozzle_in_pepper, og_nozzle_mask, params):
    debug = {}
    ...
    nozzle_mask_de_noise_open_params = depth_measurement_config["nozzle_mask_de_noise_open_params"]
    outer_mask_dilate_params = depth_measurement_config["outer_mask_dilate_params"]
    nozzle_mask_extended_outer_dilate_params = depth_measurement_config["nozzle_mask_extended_outer_dilate_params"]
    nozzle_mask_extended_inner_dilate_params = depth_measurement_config["nozzle_mask_extended_inner_dilate_params"]
    ...
    match section:
        case "top_left":
            center = (100, 100)
            cv2.line(line_mask, (center[0]-100, center[1]+100), (center[0]+100, center[1]-100), 255, line_width) #TODO: param width
        case "top_right":
            center = (100, 100)
            cv2.line(line_mask, (center[0]-100, center[1]-100), (center[0]+100, center[1]+100), 255, line_width) #TODO: param width
        case "bottom_left":
    ...
    nozzle_mask = cv2.morphologyEx(nozzle_mask, cv2.MORPH_OPEN, np.ones(nozzle_mask_de_noise_open_params["kernel"], np.uint8), iterations=nozzle_mask_de_noise_open_params["iterations"]) #TODO: param
    ...
    kernel_outer_mask = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, outer_mask_dilate_params["kernel"]) #TODO: param
    near_outer_zone_mask = cv2.dilate(nozzle_hole_mask, kernel_outer_mask, iterations=outer_mask_dilate_params["near_iterations"]) #TODO: param
    far_outer_zone_mask = cv2.dilate(nozzle_hole_mask, kernel_outer_mask, iterations=outer_mask_dilate_params["far_iterations"]) #TODO: param
    
    outer_zone_mask_raw = (far_outer_zone_mask - cv2.bitwise_and(near_outer_zone_mask, far_outer_zone_mask))
    
    kernel_nozzle = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, nozzle_mask_extended_outer_dilate_params["kernel"]) #TODO: param
    nozzle_mask_extended_outer = cv2.dilate(nozzle_mask, kernel_nozzle, iterations=nozzle_mask_extended_outer_dilate_params["iterations"]) #TODO: param
    
    kernel_nozzle = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, nozzle_mask_extended_inner_dilate_params["kernel"]) #TODO: param
    nozzle_mask_extended_inner = cv2.dilate(nozzle_mask, kernel_nozzle, iterations=nozzle_mask_extended_inner_dilate_params["iterations"]) #TODO: param
    
    debug["nozzle_mask_extended_inner"] = nozzle_mask_extended_inner
    
    outer_zone_mask = outer_zone_mask_raw - cv2.bitwise_and(outer_zone_mask_raw, nozzle_mask_extended_outer)
    outer_zone_mask = outer_zone_mask - cv2.bitwise_and(outer_zone_mask, np.invert(pepper_mask))
    outer_zone_mask = outer_zone_mask - cv2.bitwise_and(outer_zone_mask, get_mask_edge_area(convex_hull_mask(pepper_mask))) #TODO: WHY?
    ...
    kernel_inner_mask = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, inner_zone_erode_params["kernel"]) #TODO: param
    inner_zone_mask = cv2.erode(inner_zone_mask, kernel_inner_mask, iterations=inner_zone_erode_params["iterations"]) #TODO: param iters
    ...
    if inner_zone_mask is not None:
        inner_zone_mask = inner_zone_mask - cv2.bitwise_and(inner_zone_mask, nozzle_mask)
    else:
        inner_zone_mask = np.zeros_like(nozzle_mask)

    if outer_zone_mask is not None:
        outer_zone_mask = outer_zone_mask - cv2.bitwise_and(outer_zone_mask, nozzle_mask)
    else:
...
def create_overflow_mask(pepper_mask, hole_mask, params):
    ...
    outside_of_pepper_mask = cv2.bitwise_and(pepper_mask, np.invert(convex_hull_mask(hole_mask)))
    debug['outside_of_pepper_mask_init'] = outside_of_pepper_mask
    
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, kernel) 
    overflow_mask = cv2.erode(outside_of_pepper_mask, kernel, iterations=erode_iterations) 
...
def create_outer_overflow_mask(pepper_mask, hole_mask, params):
    ...
    pepper_mask_hull_erode = cv2.erode(pepper_mask_hull, np.ones(kernel, np.uint8), iterations=iter) #TODO: param
    
    outer_pepper_mask = pepper_mask - np.bitwise_and(pepper_mask, pepper_mask_hull_erode)
    hole_mask_hull = convex_hull_mask(hole_mask)
    outer_pepper_mask = outer_pepper_mask - np.bitwise_and(outer_pepper_mask, hole_mask_hull)
    
    return outer_pepper_mask, debug
    
    
#MARK: ADD NOZZLE MASKS
def add_nozzle_masks(inner_zone_mask, overflow_mask, nozzle_in_hole_mask, nozzle_outside_mask):
    debug = {}
    
    inner_zone_color = np.bitwise_or(inner_zone_mask, nozzle_in_hole_mask) - np.bitwise_and(inner_zone_mask, nozzle_outside_mask)
