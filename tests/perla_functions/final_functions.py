...
from .search_functions import pepper_presence_from_depth
from .search_functions import hole_detection
from .search_functions import create_overflow_mask
from .search_functions import split_nozzle_mask
from .search_functions import depth_measurement_zones
from .search_functions import add_nozzle_masks
...
from .fill_functions import depth_measurement_in_mask
from .fill_functions import if_pepper_is_filled
from .fill_functions import if_pepper_mask_is_white
...
#MARK: CONFIG
def config(nozzle_mask, section, pepper_type="big_pepper", reflective_nozzle = False): #small_prime, big_pepper
    ...
    match section:
        case "bottom_right":    vectors =  (data['bottom_right'][0], data['bottom_right'][1])
        case "bottom_left":     vectors =  (data['bottom_left'][0], data['bottom_left'][1])
        case "top_right":       vectors =  (data['top_right'][0], data['top_right'][1])
        case "top_left":        vectors =  (data['top_left'][0], data['top_left'][1])
    ...
    depth_measurement_zones_dict["nozzle_mask_de_noise_open_params"] = {"kernel": (5, 5), "iterations": 1}
    depth_measurement_zones_dict["outer_mask_dilate_params"] = {"kernel": (3, 3), "near_iterations": 1, "far_iterations": 14}
    depth_measurement_zones_dict["nozzle_mask_extended_outer_dilate_params"] = {"kernel": (5, 5), "iterations": 4}
    depth_measurement_zones_dict["nozzle_mask_extended_inner_dilate_params"] = {"kernel": (4, 4), "iterations": 2}
    depth_measurement_zones_dict["inner_zone_erode_params"] = {"kernel": (3, 3), "iterations": 2}
...
def create_masks(rgb, depth, nozzle_mask, params):
    ...
    pepper_presence, debug_presence = pepper_presence_from_depth(depth, pepper_mask, params)
    ...
    if not pepper_presence:
        ...
        return False, zero_mask, zero_mask, zero_mask, zero_mask, zero_mask, zero_mask, debug


    hole_mask, hole_mask_founded, debug_hole_detection = hole_detection(rgb, pepper_mask, params)
    ...
    nozzle_in_pepper, nozzle_in_hole, nozzle_outside, debug_split_nozzle = split_nozzle_mask(nozzle_mask, hole_mask, pepper_mask, params["nozzle_vectors"], params["section"])
    inner_zone_mask, outer_zone_mask, debug_zones = depth_measurement_zones(hole_mask, pepper_mask, nozzle_in_hole, nozzle_in_pepper, nozzle_mask, params)
    ...
    inner_zone_for_color, overflow_mask, debug_add_nozzles = add_nozzle_masks(inner_zone_mask, overflow_mask, nozzle_in_hole, nozzle_outside)
    ...
    return True, inner_zone_mask, outer_zone_mask, overflow_mask, outer_overflow_mask, inner_zone_for_color, exclusion_mask, debug
...
def search(rgb, depth, nozzle_mask, params):
    ...
    search_state, inner_zone_mask, outer_zone_mask, overflow_mask, outer_overflow_mask, inner_zone_for_color, exclusion_mask, debug_masks = create_masks(rgb, depth, nozzle_mask, params)
    
    debug["inner_zone_mask"] = inner_zone_mask
    debug["outer_zone_mask"] = outer_zone_mask
    debug["overflow_mask"] = overflow_mask
    debug["inner_zone_for_color"] = inner_zone_for_color
    debug["exclusion_mask"] = exclusion_mask
    debug["masks"] = debug_masks
    
    if not search_state:
        return search_state, False, inner_zone_mask, outer_zone_mask, overflow_mask, inner_zone_for_color, outer_overflow_mask, 0, 0, 0, debug
    
    masks = [inner_zone_mask, outer_zone_mask, overflow_mask, outer_overflow_mask, inner_zone_for_color, debug_masks["hole_mask"]]
    
    excluded_masks = exclude_masks(masks, exclusion_mask)
    
    debug["final_hole_mask"] = excluded_masks[5]
    outer_zone_median_start, _, _ = depth_measurement_in_mask(depth, outer_zone_mask)
    
    overflow_state = True
    min_size = 50 #TODO: param
    if check_mask_size(inner_zone_mask, params["min_mask_size_config"]["inner_zone_mask"]): search_state = False
    if check_mask_size(outer_zone_mask, params["min_mask_size_config"]["outer_zone_mask"]): search_state = False
    if check_mask_size(overflow_mask, params["min_mask_size_config"]["overflow_mask"]): overflow_state = False
    if check_mask_size(inner_zone_for_color, params["min_mask_size_config"]["inner_zone_for_color"]): search_state = False
    ...
    overflow_outer_bias = get_white_percentage_in_mask(rgb, outer_overflow_mask, hsv_range)[0]
                                                                                                                #4
    return search_state, overflow_state, excluded_masks[0], excluded_masks[1], excluded_masks[2], excluded_masks[0], excluded_masks[3], outer_zone_median_start, overflow_bias, overflow_outer_bias, debug

#MARK: FILL
def fill(rgb, depth, inner_zone_mask, outer_zone_mask, inner_zone_for_color, outer_zone_median_start, overflow_mask, overflow_bias, overflow_outer_mask, overflow_outer_bias, params, overflow_only=False):
    debug = {}
    ...
    if not overflow_only:
        inner_zone_median, inner_zone_non_zero_perc, debug_inner = depth_measurement_in_mask(depth, inner_zone_mask)
        outer_zone_median, outer_zone_non_zero_perc, debug_outer = depth_measurement_in_mask(depth, outer_zone_mask)

        debug["inner_zone_median"] = inner_zone_median
        debug["inner_zone_non_zero_perc"] = inner_zone_non_zero_perc
        debug["outer_zone_median"] = outer_zone_median
        debug["outer_zone_non_zero_perc"] = outer_zone_non_zero_perc
        debug["debug_inner"] = debug_inner
        debug["debug_outer"] = debug_outer
        
        is_filled, debug_filled = if_pepper_is_filled(inner_zone_median, inner_zone_non_zero_perc, outer_zone_median, outer_zone_median_start, params)

        debug["is_filled"] = is_filled
        debug["debug_filled"] = debug_filled
        
        white_good, debug_white = if_pepper_mask_is_white(rgb, inner_zone_for_color, params)
        
        debug["white_good"] = white_good
        debug["debug_white"] = debug_white
    ...
    is_overflow, debug_overflow = overflow_detection(rgb, overflow_mask, hsv_range, params["overflow_detection_config"]["inner_overflow_max_perc"], overflow_bias)    
    is_overflow_outer, debug_overflow_outer = overflow_detection(rgb, overflow_outer_mask, hsv_range, params["overflow_detection_config"]["outer_overflow_max_perc"], overflow_outer_bias)
    
    debug["is_overflow"] = is_overflow
    debug["debug_overflow"] = debug_overflow
    debug["is_overflow_outer"] = is_overflow_outer
    debug["debug_overflow_outer"] = debug_overflow_outer
        
    if is_overflow or is_overflow_outer:
        pepper_overflow = True
        
    return pepper_filled, pepper_overflow, debug