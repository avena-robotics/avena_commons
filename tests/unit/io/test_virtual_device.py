...
from avena_commons.io.virtual_device.virtual_device import VirtualDevice, VirtualDeviceState
...
class TestVirtualDevice:
    ...
    def test_handle_check_state_event_custom_enum(self, mock_debug, virtual_device):
        ...
        assert result_event.data["state_name"] == CustomTestDeviceState.CUSTOM_STATE.name
    ...
    def test_move_event_to_finished_success(self, mock_debug, virtual_device):
        ...
        result = virtual_device._move_event_to_finished("test_event", "success", "Test message")
    ...
    def test_move_event_to_finished_no_message(self, mock_debug, virtual_device):
        ...
        assert result is True
        assert virtual_device._finished_events[0].result.result == "success"
        assert not hasattr(virtual_device._finished_events[0].result, 'error_message') or \
               virtual_device._finished_events[0].result.error_message is None
    ...
    @patch('avena_commons.io.virtual_device.virtual_device.MeasureTime')
    def test_execute_event_measure_time_context(self, mock_measure_time_class, virtual_device):
