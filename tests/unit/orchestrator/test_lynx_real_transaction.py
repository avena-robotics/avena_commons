...
async def test_real_lynx_transaction():
    ...
    config = {"SITE_ID": str(REAL_SITE_ID), "ACCESS_TOKEN": access_token}

    # Utwórz komponent
    component = LynxAPIComponent(name="test_real_lynx_api", config=config)
    ...
    try:
        ...
        print(
            f"   {'✅' if is_valid else '❌'} Walidacja: {'OK' if is_valid else 'BŁĄD'}"
        )
        ...
        print(
            f"   {'✅' if is_initialized else '❌'} Inicjalizacja: {'OK' if is_initialized else 'BŁĄD'}"
        )
        ...
        print(
            f"   {'✅' if is_connected else '❌'} Połączenie: {'OK' if is_connected else 'BŁĄD'}"
        )
        ...
        print(f"   Site ID: {REAL_SITE_ID}")
        print(
            f"   URL: {component.get_base_url()}/operational/v1/payment/refund-request"
        )

        response = (
            input("\nCzy kontynuować wysyłanie żądania refund? (tak/nie): ")
            .strip()
            .lower()
        )
...
async def test_lynx_component_validation_only():
    ...
    component = LynxAPIComponent(name="validation_test", config=config)
