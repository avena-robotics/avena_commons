...
async def test_database_list_condition_basic():
    ...
    mock_db.fetch_records = AsyncMock(
        return_value=[
    ...
    # Mock context
    context = {"components": {"test_db": mock_db}, "trigger_data": {}}
    ...
    assert "test_orders" in context["trigger_data"], (
        "Dane powinny być zapisane w trigger_data"
    )
...
async def test_database_list_condition_empty_result():
    ...
    config = {
        ...
        "table": "zamowienia",
        "columns": ["id"],
        "where": {"stan_zamowienia": "non_existent"},
