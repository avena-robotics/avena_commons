...
class AutonomousPepperCameraBenchmark:
    ...
    def __init__(self, duration_sec: int = 60, monitoring_frequency: int = 30):
    ...
    def get_system_stats(self):
        ...
        try:
            ...
            cpu_percpu = psutil.cpu_percent(
                percpu=True, interval=self.monitoring_interval
            )
            camera_core_usage = (
                cpu_percpu[self.camera_core]
                if self.camera_core < len(cpu_percpu)
                else 0
            )
            pepper_core_usage = (
                cpu_percpu[self.pepper_core]
                if self.pepper_core < len(cpu_percpu)
                else 0
            )
        except:
            camera_core_usage = 0
            ...
            if self.pepper_listener:
                return {
                    "is_processing": getattr(
                        self.pepper_listener, "_is_processing", False
                    ),
                    "processing_enabled": getattr(
                        self.pepper_listener, "processing_enabled", True
                    ),
                    "fragment_buffer_size": len(
                        getattr(self.pepper_listener, "fragment_buffer", [])
                    ),
                    "expected_fragments": getattr(
                        self.pepper_listener, "expected_fragments", 4
                    ),
                    "last_result": getattr(
                        self.pepper_listener, "last_processing_result", None
                    ),
                    "pepper_state": str(
                        getattr(
                            self.pepper_listener.pepper_connector,
                            "get_state",
                            lambda: "unknown",
                        )()
                    ),
    ...
    async def setup_autonomous_eventlisteners(self):
        ...
        print(
            "📋 Order: Pepper first (ready to receive) → PepperCamera second (starts autonomous processing)"
        )
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}:{port1}/event", json=event1.to_dict()
            )
            ...
            response = requests.post(
                f"{self.base_url}:{port2}/event", json=event2.to_dict()
            )
            print(f"Response status code: {response.status_code}")
            ...
            print(
                f"   📷 PepperCamera: localhost:8001 → {self.camera_ip} (Core {self.camera_core})"
            )
            print(
                "📊 PepperCamera will continuously: grab frame → fragment → send to Pepper"
            )
            print(
                "🌶️  Pepper will continuously: receive fragments → buffer → process when ready"
            )
            print(
                f"🔍 Monitoring at {self.monitoring_frequency}Hz for {self.duration_sec}s"
            )
        ...
        try:
            while time.perf_counter() - start_time < self.duration_sec:
                ...
                if current_time >= next_sample_time:
                    ...
                    self.stats["cpu_usage_camera_core"].append(
                        sys_stats["cpu_camera_core"]
                    )
                    self.stats["cpu_usage_pepper_core"].append(
                        sys_stats["cpu_pepper_core"]
                    )
                    ...
                    if (
                        int(elapsed) % 10 == 0
                        and elapsed > 0
                        and sample_count % (self.monitoring_frequency * 10) == 0
                    ):
                        print(
                            f"📊 Monitoring {elapsed:.0f}s | Samples: {sample_count} | Camera core {self.camera_core}: {sys_stats['cpu_camera_core']:.1f}% | Pepper core {self.pepper_core}: {sys_stats['cpu_pepper_core']:.1f}% | Memory: {sys_stats['memory_mb']:.0f}MB"
                        )
    ...
    def analyze_results(self):
        ...
        if self.stats["pepper_processing_status"]:
            ...
            processing_percentage = (
                processing_count / len(self.stats["pepper_processing_status"])
            ) * 100
        ...
        if self.stats["camera_processing_status"]:
            ...
            camera_processing_percentage = (
                camera_processing_count / len(self.stats["camera_processing_status"])
            ) * 100
            print(
                f"   Camera processing:     {camera_processing_percentage:.1f}% of time"
            )
...
async def main():
    ...
    try:
        ...
        print(
            f"🚀 Starting {benchmark.duration_sec}s autonomous monitoring at {benchmark.monitoring_frequency}Hz..."
        )
