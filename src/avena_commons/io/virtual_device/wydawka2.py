...
class Wydawka2(VirtualDevice):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"{self.device_name} - Wydawka2 initialized with steps: {self.steps})")
        print(f"{self.device_name} -  max_velocity: {self.__max_velocity}")
        print(f"{self.device_name} -  acceleration: {self._acceleration}")
        print(f"{self.device_name} -  deceleration: {self._deceleration} ")
        print(f"{self.device_name} -  segments: {self.__veloctiy_segments} ")
    ...
    def __move_forward(self, **kwargs):
        ...
        #     continue
        return getattr(self.devices[kwargs["device"]], kwargs["method"])(speed=kwargs["speed"], accel = self._acceleration, decel=self._deceleration)

    def __stop(self, **kwargs):
        return getattr(self.devices[kwargs["device"]], kwargs["method"])(decel=self._deceleration)
    ...
    def _on_encoder_timeout(self, task: SensorTimerTask) -> None:
        ...
        if self.__fsm == WydawkaState2.RUNNING:
            self.__stop(deccel= self._deceleration, **self.methods["stop"])
    ...
    def tick(self):
        ...
        if self._processing_events.get("wydawka_maintenance_enable"):
            self.__fsm = WydawkaState2.ENABLING_MAINTENANCE
        ...
        match self.__fsm:
            ...
            case WydawkaState2.RUNNING:
                ...
                print(f"{self.device_name} - RUNNING {encoder_position - self.__starting_position} > {self.__target_steps} | encoder position {encoder_position} and {self.__starting_position}")
                debug(f"{self.device_name} - RUNNING {encoder_position - self.__starting_position} > {self.__target_steps} | encoder position {encoder_position} and {self.__starting_position}", message_logger=self._message_logger)
                
                if encoder_position - self.__starting_position < 0:
                    warning(f"{self.device_name} - Wydawka zaczyna ze złej pozycji Pozycja enkodera: {encoder_position}, pozycja startowa: {self.__starting_position}", message_logger=self._message_logger)
                    self.__starting_position = encoder_position
                ...
                if encoder_position - self.__starting_position >= self.__target_steps:
                    ...
                    self.__fsm = WydawkaState2.STOPPING
                    self._move_event_to_finished(event_type="wydawka_move", result="success")

                else:
                    print(f"{self.device_name} - RUNNING checking speed segments, index: {self.__veloctiy_segments_index}")
                    match self.__veloctiy_segments_index:
                        case 0:
                            print(f"{self.device_name} - RUNNING segment 0 check: {self.__starting_position - encoder_position} >= {self.__target_steps * self.__veloctiy_segments[1][0]}")
                            if abs(self.__starting_position - encoder_position) >= self.__target_steps * self.__veloctiy_segments[1][0]:
                                self.__veloctiy_segments_index = 1
                                # self.__stop(**self.methods["stop"])
                                self.__move_forward(speed=int(self.__max_velocity * self.__veloctiy_segments[1][1]), **self.methods["move_forward"])
                                if self.__debug:
                                    debug(f"{self.device_name} - zmiana prędkości na {self.__veloctiy_segments[1][1]}", message_logger=self._message_logger)
                            
                        case 1:
                            if abs(self.__starting_position - encoder_position) >= self.__target_steps * self.__veloctiy_segments[2][0]:
                                self.__veloctiy_segments_index = 2
                                # self.__stop(**self.methods["stop"])
                                self.__move_forward(speed=int(self.__max_velocity * self.__veloctiy_segments[2][1]), **self.methods["move_forward"])
                                if self.__debug:
                                    debug(f"{self.device_name} - zmiana prędkości na {self.__veloctiy_segments[2][1]}", message_logger=self._message_logger)
                        case 2:
                            if abs(self.__starting_position - encoder_position) >= self.__target_steps * self.__veloctiy_segments[3][0]:
                                self.__veloctiy_segments_index = 3
                            
                                # self.__stop(**self.methods["stop"])
                                self.__move_forward(speed=int(self.__max_velocity * self.__veloctiy_segments[3][1]), **self.methods["move_forward"])
                                if self.__debug:
                                    debug(f"{self.device_name} - zmiana prędkości na {self.__veloctiy_segments[3][1]}", message_logger=self._message_logger)
            case WydawkaState2.STOPPING:
                is_motor_running = self.__is_motor_running(**self.methods["is_motor_running"])
                if not is_motor_running:
                    ...
                    print(f"{self.device_name} STOP Pozycją końcowa: {encoder_position}, dystans przebyty: {actual_distance}, docelowy dystans: {self.__target_steps} pozycja początkowa {self.__starting_position}")
                    self.__encoder_correct = actual_distance - self.__target_steps  # Target_steps bug fix
                    print(f"{self.device_name} STOP Korekta kroków: {self.__encoder_correct}")
                    if self.__debug:
                        debug(
            ...
            case WydawkaState2.BACK:
                print(f"{self.device_name} korekta kroków: {self.__encoder_correct} > {self.__max_correction_steps}, ruch w tył")
                self.__move_forward(speed=self._back_velocity, **self.methods["move_forward"])  # FIXME
                actual_distance = encoder_position - self.__starting_positio
                self.__encoder_correct = actual_distance - self.__target_steps  # Target_steps bug fix
            ...
            case WydawkaState2.ENABLING_MAINTENANCE:
                ...
                if self._processing_events.get("wydawka_maintenance_enable"):
                    ...
                    self.__fsm = WydawkaState2.MAINTENANCE

            case WydawkaState2.MAINTENANCE:
                #
