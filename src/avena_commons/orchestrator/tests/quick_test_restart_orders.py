...
async def test_restart_orders_simulation():
    ...
    async def mock_select_list(table, columns, where_conditions=None, **kwargs):
        ...
        if table == "aps_order_item":
            ...
            if order_id == 1001:
                return [
                    {"id": 3001, "aps_order_id": 1001, "item_id": 101, "status": "pending", "aps_id": 2001},
                    {"id": 3002, "aps_order_id": 1001, "item_id": 101, "status": "pending", "aps_id": 2001}, # 2x kawa
                    {"id": 3003, "aps_order_id": 1001, "item_id": 102, "status": "pending", "aps_id": 2001}  # 1x ciastko
                ]
            elif order_id == 1002:
                return [
                    {"id": 3004, "aps_order_id": 1002, "item_id": 103, "status": "pending", "aps_id": 2002}, # 1x herbata
                    {"id": 3005, "aps_order_id": 1002, "item_id": 104, "status": "pending", "aps_id": 2002}  # 1x muffin (niedostępny)
                ]
        ...
        # Stan magazynu
            ...
            storage_state = {
                101: [{"current_quantity": 10, "slot_name": "A1-Kawa"}],      # Dostępne: kawa
                102: [{"current_quantity": 5, "slot_name": "B1-Ciastka"}],    # Dostępne: ciastko
                103: [{"current_quantity": 8, "slot_name": "C1-Herbata"}],    # Dostępne: herbata
                104: [{"current_quantity": 0, "slot_name": "D1-Muffin"}]      # Niedostępne: muffin
            }
            return storage_state.get(item_id, [{"current_quantity": 0, "slot_name": "Unknown"}])
    ...
    try:
        ...
        for detail in result["details"]:
            status = "✅ SKLONOWANO" if detail["success"] else "🔄 REFUND" if not detail["error"] else "❌ BŁĄD"
            print(f"  {status} - Zamówienie {detail['order_id']} (APS: {detail['aps_id']})")
            if detail.get("availability"):
                items = detail["availability"]["items"]
                for item in items:
                    availability = "✅" if item["is_available"] else "❌"
                    print(f"    {availability} Produkt {item['item_id']}: {item['required_count']} szt (magazyn: {item['available_quantity']})")
...
async def test_configuration_scenarios():
    ...
    print(f"  pickup_number pominięty: {'pickup_number' not in clone_config['copy_fields']}")
    
    # Test 3: Konfiguracja z wartościami domyślnymi
    print("\n3️⃣ Test z wartościami domyślnymi:")
    mock_config = {
        "clone_config": {
            "default_values": {"pickup_number": None, "status": "pending"}
        }
    }
    clone_config = action._get_clone_configuration(mock_config)
    print(f"  Wartości domyślne: {clone_config['default_values']}")
