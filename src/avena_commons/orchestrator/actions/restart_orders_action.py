...
class RestartOrdersAction(BaseAction):
    ...
    async def execute(self, action_config: Dict[str, Any], context: ActionContext) -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    if availability_check["all_available"]:
                        ...
                        new_order_id = await self._process_successful_restart(
                            db_component, order, availability_check, clone_config, context
                        )
                        ...
                        await self._process_failed_restart(db_component, order_id, context)
    ...
    def _get_orders_from_context(self, action_config: Dict[str, Any], context: ActionContext) -> List[Dict[str, Any]]:
        ...
        orders = trigger_data.get(orders_key.replace("{{ trigger.", "").replace(" }}", ""))
        
        if not isinstance(orders, list):
            raise ActionExecutionError(
                self.action_type, f"Dane z '{orders_source}' nie są listą: {type(orders)}"
            )
            
        return orders
    
    def _get_database_component(self, action_config: Dict[str, Any], context: ActionContext):
    ...
    def _get_clone_configuration(self, action_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        default_copy_fields = [
            "aps_id", "origin", "status", "kds_order_number", 
            "client_phone_number", "estimated_time", "transaction_id",
            "marketing_consent", "terms_accepted", "privacy_accepted", "promo_consent"
        ]
    ...
    async def _check_order_items_availability(self, db_component, order_id: int, aps_id: int) -> Dict[str, Any]:
        """
    ...
    async def _process_successful_restart(self, db_component, order: Dict, availability_check: Dict, 
                                        clone_config: Dict, context: ActionContext) -> int:
        """
        ...
        async with db_component._connection.transaction():
            ...
            new_order_id = await self._clone_aps_order(db_component, order, clone_config)
    ...
    async def _process_failed_restart(self, db_component, order_id: int, context: ActionContext):
        """
    ...
    async def _clone_aps_order(self, db_component, order: Dict, clone_config: Dict) -> int:
        """
    ...
    async def _clone_aps_order_item(self, db_component, original_item_id: int, 
                                   new_order_id: int, aps_id: int):
        """
