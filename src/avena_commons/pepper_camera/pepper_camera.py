...
class PepperCamera(EventListener):
    ...
    def __init__(
        ...
        if self.__camera_config.get("camera_ip", None) is None:
            error(
                f"Missing CAMERA_IP configuration for pepper camera",
        ...
        debug(
            f"PepperCamera EventListener initialized for IP {self.camera_address}",
            self._message_logger,
        )
    ...
    async def on_initializing(self):
        ...
        full_config = {
            **self.__camera_config,
            "camera_pipeline": self.__pipeline_config,
        }
    ...
    def _log_performance_metrics(self):
        ...
        try:
            ...
            metrics_summary = {
                ...
                "processing_stats": {
                    "total_frames_processed": self.performance_metrics[
                        "frames_processed"
                    ],
            ...
            if self.performance_metrics["frames_processed"] > 0:
                avg_total = (
                    sum(self.performance_metrics["total_processing_times"])
                    / len(self.performance_metrics["total_processing_times"])
                    if self.performance_metrics["total_processing_times"]
                    else 0
                )
                info(
                    f"PERFORMANCE SUMMARY - Frames: {self.performance_metrics['frames_processed']}, Avg Total Time: {avg_total:.2f}ms",
                    self._message_logger,
                )
        ...
        match camera_state:
            ...
            case CameraState.STARTED:
                ...
                if last_frame is not None:
                    debug(
                        f"Got frame: color {last_frame['color'].shape}, depth {last_frame['depth'].shape}",
                        self._message_logger,
                    )
                    ...
                    if not confirmed:
                        error(
                            "Error in fragment_and_serialize_frame",
                            self._message_logger,
                        )
                        ...
                        debug(
                            f"Frame fragmentation initiated in {processing_time.ms:.2f}ms",
                            self._message_logger,
                        )
                        ...
                        if fragments_data is not None:
                            debug(
                                f"Got processed fragments: {len(fragments_data)}",
                                self._message_logger,
                            )

                            # Send fragments to Pepper EventListener
                            fragments_sent = (
                                await self._send_fragments_to_pepper_listener(
                                    fragments_data
                                )
                            )
                            ...
                            if fragments_sent:
                                debug(
                                    f"Successfully sent {len(fragments_data)} fragments to Pepper EventListener (autonomous)",
                                    self._message_logger,
                                )
                                self.performance_metrics["fragments_sent"] += (
                                    len(fragments_data)
                                ...
                                error(
                                    "Failed to send fragments to Pepper EventListener (autonomous)",
                                    self._message_logger,
                                )
                                self.performance_metrics["processing_errors"] += 1
                            ...
                            debug(
                                "Camera continues running for next frame",
                                self._message_logger,
                            )
                        else:
                            error(
                                "No fragments data available after processing",
                                self._message_logger,
                            )
                            self.performance_metrics["processing_errors"] += 1
                else:
