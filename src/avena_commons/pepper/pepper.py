...
class Pepper(EventListener):
    ...
    def __init__(
        ...
        if not port:
            raise ValueError(
                "Brak wymaganej zmiennej środowiskowej PEPPER_LISTENER_PORT"
            )
        ...
        self.expected_fragments = self.__pepper_config.get(
            "expected_fragments", 4
        )  # Expecting 4 fragments per frame
    ...
    def _log_performance_metrics(self):
        ...
        try:
            ...
            metrics_summary = {
                ...
                "processing_stats": {
                    "total_fragments_processed": self.performance_metrics[
                        "fragments_processed"
                    ],
                    "total_processing_sessions": self.performance_metrics[
                        "processing_sessions"
                    ],
            ...
            if self.performance_metrics["processing_sessions"] > 0:
                ...
                info(
                    f"PERFORMANCE SUMMARY - Sessions: {self.performance_metrics['processing_sessions']}, Fragments: {self.performance_metrics['fragments_processed']}, Avg Total Time: {avg_total:.2f}ms",
                    self._message_logger,
                )
    ...
    def _deserialize_fragments(
        ...
        with Catchtime() as deser_timer:
            try:
                ...
                for fragment_name, fragment_data in fragments_data.items():
                    ...
                    for key, value in fragment_data.items():
                        if (
                            ...
                            array_bytes = base64.b64decode(
                                value["data"].encode("utf-8")
                            )
                            dtype = np.dtype(value["dtype"])
                            shape = tuple(value["shape"])

                            array = np.frombuffer(array_bytes, dtype=dtype).reshape(
                                shape
                            )
                            fragment[key] = array
                        else:
                    ...
                    debug(
                        f"Deserialized fragment {fragment_name} with keys: {list(fragment.keys())}",
                        self._message_logger,
                    )
                ...
                error(
                    f"Error deserializing fragments from JSON: {e}",
                    self._message_logger,
                )
