    ...
    try:
        ...
        if cuda_count > 0:
            if init_device:
                ...
                return (
                    True,
                    f"CUDA available: {cuda_count} device(s), using device {device_name}",
                )
            else:
...
class GPUBatchProcessor:
    ...
    def batch_color_convert(
        ...
        if not self.cuda_available:
            # CPU fallback
            cpu_images = [
                img if isinstance(img, np.ndarray) else img.download()
        ...
        for idx, gpu_img in enumerate(gpu_images):
            stream = (
                self.streams[idx % len(self.streams)]
                if use_streams and self.streams
                else None
            )
    ...
    def batch_inrange(
        ...
        if not self.cuda_available:
            ...
            return [
                cv2.inRange(img, np.array(lower), np.array(upper))
                for img, lower, upper in zip(cpu_images, lower_bounds, upper_bounds)
            ]

        masks = []
        for idx, (gpu_img, lower, upper) in enumerate(
            zip(gpu_images, lower_bounds, upper_bounds)
        ):
            stream = (
                self.streams[idx % len(self.streams)]
                if use_streams and self.streams
    ...
    def batch_morphology(
        ...
        if not self.cuda_available:
            ...
            return [
                cv2.morphologyEx(img, operation, kernel, iterations=iterations)
    ...
    def batch_threshold(
        ...
        if not self.cuda_available:
            ...
            return [
                cv2.threshold(img, thresh, maxval, threshold_type)[1]
                for img, thresh, maxval in zip(cpu_images, thresholds, max_values)
            ]

        thresholded = []
        for idx, (gpu_img, thresh, maxval) in enumerate(
            zip(gpu_images, thresholds, max_values)
        ):
            stream = (
                self.streams[idx % len(self.streams)]
                if use_streams and self.streams
            ...
            cv2.cuda.threshold(
                gpu_img, thresh, maxval, threshold_type, result, stream=stream
            )
